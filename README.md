...
int count_near_due() {
    ...
    for (int i = 0; i < BCOUNT; i++) {
        if (strcmp(books[i][1], "대출중") == 0) {
            double elapsed = difftime(now, borrow_times[i]);
            if (elapsed >= BORROW_TIME_LIMIT - 30 && elapsed < BORROW_TIME_LIMIT) {
                count++;
            }
        }
    }
